...
def proxy_kernel_linear(X, Y, K=linear):
...
def linear_predict(X_test, X_train, Y_train):
    #One-vs-Rest
    out = []
    output = []
    coeff = []
    ...
    for i in range(len(np.unique(Y_train))):
        ...
        print "predict" + str(i)
        clf = svm.SVC(kernel =proxy_kernel_linear).fit(X_train,Y)
    ...
    for j in range(len(np.unique(Y_train))):
        gram_mat = proxy_kernel_linear(X_train[sv[j]],X_test)
