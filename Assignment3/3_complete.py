...
for i in itr:
    mlp = MLPClassifier(activation='logistic', hidden_layer_sizes=(500, 300, 100, 30), verbose=False, max_iter=i, early_stopping=False)
    mlp.fit(pixel.reshape((pixel.shape[0], -1)), label)
    acc = mlp.score(test_pixel.reshape((test_pixel.shape[0], -1)), test_label)
    print 'Score:::', acc
    tot.append(acc)

#wpath = 'Weights/' + '3_complete' + '_' + 'logistic_(500,300,50)' + '.pkl'
...
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.savefig('Plots/3_complete_tanh_50030080_ittr.png')
plt.show()

'''
wpath = 'Weights/' + '1C' + '_' + 'relu_softmax_complete' + '.pkl'
